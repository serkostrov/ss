/**
 * Same-origin INN → company lookup for production (nginx proxies /api/company-by-inn here).
 * Mirrors apps/web/scripts/inn-lookup-middleware.ts and the edge function.
 */
import http from 'node:http'

const PORT = Number(process.env.INN_LOOKUP_PORT || 3099)
...
async function lookupByInn(inn) {
  ...
  if (!startRes.ok) throw new Error(`fns_start_${startRes.status}`)
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname !== '/api/company-by-inn' && url.pathname !== '/') {
    sendJson(res, 404, { error: 'not_found' })
    return
  }

  // Health for docker / nginx
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`[inn-lookup] listening on 127.0.0.1:${PORT}`)
})
